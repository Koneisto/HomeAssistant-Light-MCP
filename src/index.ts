...
const tools: Tool[] = [
  ...
  },
  {
    name: "scene_create",
    description:
      "Create a new scene in Home Assistant by capturing current light states. Ask user: 1) Should this be 'exclusive' (turns off other lights when activated) or 'additive' (only affects lights in scene)?",
    inputSchema: {
      type: "object",
      properties: {
        name: {
        ...
        mode: {
          ...
          description: "Scene mode: 'exclusive' turns off lights not in scene, 'additive' only sets lights in scene without affecting others",
        },
        entity_ids: {
          type: "array",
          items: { type: "string" },
  ...
  {
    name: "scene_blackout",
    description: "Turn off ALL lights. Optionally creates a 'Blackout' scene first.",
    inputSchema: {
      type: "object",
      properties: {
        create_scene: {
          type: "boolean",
          description: "If true, creates a 'Blackout' scene with all lights off before turning them off.",
        },
      },
    },
  },
...
async function handleActivateScene(args: { entity_id: string }): Promise<string> {
  ...
  const sceneEntityIds = scene.attributes.entity_id || [];
  ...
  // Turn off lights not in the scene if exclusive mode
  if (mode === "exclusive" && sceneEntityIds.length > 0) {
    const allLights = await getLights();
    const lightsToTurnOff = allLights
      .filter((l) => l.state === "on" && !sceneEntityIds.includes(l.entity_id))
      .map((l) => l.entity_id);

    if (lightsToTurnOff.length > 0) {
      await callService("light", "turn_off", { entity_id: lightsToTurnOff });
    }
  }
  ...
  for (let i = 0; i < standardLights.length; i++) {
    ...
    const lightConfig = sceneConfig.entities[lightId];
  ...
  const modeInfo = mode === "exclusive" ? " (turned off other lights)" : "";
  const ikeaInfo = ikeaLights.length > 0 ? ` (${ikeaLights.length} IKEA lights with delays)` : "";
  return `Activated scene "${entity_id}" - set ${lightsSet} lights${modeInfo}${ikeaInfo}`;
}
...
async function handleBlackout(args: { create_scene?: boolean }): Promise<string> {
  const { create_scene } = args;
  const allLights = await getLights();

  let sceneMessage = "";

  // Create "Blackout" scene if requested (all lights off)
  if (create_scene) {
    // Check if "Blackout" scene already exists
    const scenes = await getScenes();
    ...
    if (existingBlackout) {
      sceneMessage = "'Blackout' scene already exists. ";
    } else {
      const entities: Record<string, Record<string, unknown>> = {};
      for (const light of allLights) {
        entities[light.entity_id] = { state: "off" };
      }
      ...
      await saveSceneConfig(sceneConfig);
      sceneMessage = "Created 'Blackout' scene. ";
    }
  }
  ...
  // Turn off all lights
  const entityIds = lightsOn.map((l) => l.entity_id);
  await callService("light", "turn_off", { entity_id: entityIds });

  return `${sceneMessage}Blackout: Turned off ${lightsOn.length} lights.`;
}
  ...
  try {
    ...
    switch (name) {
      ...
      case "scene_update":
        ...
        break;
      case "scene_blackout":
        result = await handleBlackout(args as { create_scene?: boolean });
        break;
      default:
